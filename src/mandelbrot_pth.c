...
struct sub_image calcula_sub_image(long id) {
    struct sub_image si;
...
void *mandelbrot_th(void *thread_id) {
    int id;
    struct sub_image si;

    id = (long) thread_id;
    si = calcula_sub_image(id);
...
void cria_threads() {
    ...
    for (i = 0; i < n_cores; i++) {
        ...
        aux = pthread_create(&threads[i], NULL, mandelbrot_th, (void*) i);
        if (aux) {
